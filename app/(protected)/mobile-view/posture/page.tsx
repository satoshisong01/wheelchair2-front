...
export default function PosturePage() {
  ...
  // 🟢 [보강] '완료(✓)'가 무한히 갇히지 않도록 표시 후 자동 해제.
  //     기기 PHT가 '현재 고정'이고 시트각도(CW/st, 5초)만으로는 새 세션을 놓칠 수 있어,
  //     완료 표시 12초 뒤 게이지를 안전하게 리셋해 다음 세션을 받을 수 있게 한다.
  useEffect(() => {
    if (!isSuccessThisSession) return;
    const t = setTimeout(() => {
      queueMicrotask(() => {
