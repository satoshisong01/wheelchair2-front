...
export default function LoginPage() {
 ...
 if (status === 'loading') {
  ...
  // 3. 이미 로그인된 상태라면 (useEffect가 처리하지만 만일을 대비해) 스피너를 보여줍니다.
  // 이 코드가 있으면 authenticated일 때 잠시 스피너를 보여주지만, useEffect가 바로 리다이렉트 시킵니다.
  // 이 코드는 삭제하고, useEffect에 의존하는 것이 더 확실합니다.

  // 4. 로그인이 안 된 사용자에게만 로그인 페이지 표시
