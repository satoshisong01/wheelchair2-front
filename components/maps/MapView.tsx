...
interface MapViewProps {
  ...
  onSelectWheelchair: (e: any, wheelchair: DashboardWheelchair) => void;
}
...
export default function MapView({
  ...
  const markersRef = useRef<{ [key: number]: KakaoMapMarker }>({});
  const [isScriptLoaded, setIsScriptLoaded] = useState(false);

  // --- 🔽🔽🔽 [수정 1] `updateMarkers`를 `initializeMap`보다 먼저 선언 🔽🔽🔽 ---
  /**
  ...
  const updateMarkers = (map: KakaoMapInstance, kakao: KakaoMapsSDK) => {
    ...
    wheelchairs.forEach((wheelchair) => {
      ...
      if (currentMarkers[wheelchair.id]) {
        currentMarkers[wheelchair.id].setPosition(position);
      } else {
        const marker = new kakao.maps.Marker({
          map: map,
          position: position,
          title: wheelchair.nickname || wheelchair.deviceSerial,
        });
        kakao.maps.event.addListener(marker, 'click', () => {
          onSelectWheelchair(wheelchair);
          map.panTo(position);
        });
        currentMarkers[wheelchair.id] = marker;
      }
    });
  };
  // --- 🔼🔼🔼 [수정 1] 🔼🔼🔼 ---
  ...
  const initializeMap = () => {
    ...
    window.kakao.maps.load(() => {
      ...
      // 탭 전환 시 mapContainer가 null이 될 수 있으므로 다시 체크
      ...
      // 마커 업데이트 (초기 로드) - 이제 이 함수는 위에 선언되어 있습니다.
      updateMarkers(map, kakao);

      // [회색 지도 버그 수정] 렌더링 딜레이 후 relayout
      setTimeout(() => {
        if (mapRef.current) {
  ...
  // --- 🔽🔽🔽 [수정 2] 탭 전환 문제 해결을 위한 useEffect 추가 🔽🔽🔽 ---
  // 컴포넌트 마운트 시(탭 전환 포함) 항상 실행
  useEffect(() => {
  ...
  // [수정 3] <Script onLoad>로 인한 첫 로드를 처리하는 useEffect
  useEffect(() => {
  ...
  }, [wheelchairs, onSelectWheelchair]);

  // --- 🔽🔽🔽 [신규 추가] 휠체어 선택 시 지도로 이동 (Problem 1) 🔽🔽🔽 ---
  useEffect(() => {
    // 1. 맵이 준비되고, 휠체어가 선택되었는지 확인
    if (mapRef.current && selectedWheelchair && window.kakao) {
      const lat = selectedWheelchair.status?.latitude;
      const lng = selectedWheelchair.status?.longitude;

      // 2. 선택된 휠체어의 좌표가 있는지 확인
      if (lat && lng) {
        const position = new window.kakao.maps.LatLng(lat, lng);
        // 3. 지도를 해당 좌표로 부드럽게 이동
        mapRef.current.panTo(position);
      }
    }
  }, [selectedWheelchair]); // selectedWheelchair prop이 변경될 때마다 실행
  // --- 🔼🔼🔼 [신규 추가] 🔼🔼🔼 ---
